...
k = (numAlleles * numAlleles + numAlleles) / 2;
...
for (i = 1:k),
    for (j = 1:k),
        ...
        B = genotypesToAlleles(j, :);
        vector_alleles = [allelesToGenotypes(A(1), B(1)), allelesToGenotypes(A(1), B(2)), allelesToGenotypes(A(2), B(1)), allelesToGenotypes(A(2), B(2))]
        for (v = 1:4),
            genotypeFactor.val(AssignmentToIndex( [vector_alleles(v), i, j], genotypeFactor.card)) += .25;
        end;
    end;
end;
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
