...
for (i = 1:genotypeFactor.card)
    v1 = genotypesToAlleles(i, 1);
    v2 = genotypesToAlleles(i, 2);
    val = alleleFreqs(v1) * alleleFreqs(v2);
    if (v1 != v2),
        val = val * 2;
    end;
    genotypeFactor = SetValueOfAssignment(genotypeFactor, [allelesToGenotypes(v1, v2)], val);
end;
