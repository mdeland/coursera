...
for i = 1:length(indxA),
    x = GetValueOfAssignment(A, IndexToAssignment(indxA(i), A.card)) * GetValueOfAssignment(B, IndexToAssignment(indxB(i), B.card));
    C = SetValueOfAssignment(C, assignments(i,:), x);
end
