...
for i = 1:length(indxB),
    x = GetValueOfAssignment(A, assignments(i,:));
    y = GetValueOfAssignment(B, IndexToAssignment(indxB(i), B.card));
    B = SetValueOfAssignment(B, IndexToAssignment(indxB(i), B.card), x + y);
end
